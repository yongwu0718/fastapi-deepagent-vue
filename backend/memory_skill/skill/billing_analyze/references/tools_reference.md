...
保存单条账单到 SQLite (v3 schema)。参数：`item_name`, `category`, `amount`(金额绝对值), `date`, `platform`, `expense_type`, `direction`("支出"|"收入",默认支出)。

> `year_month` 由 `date` 自动派生，无需传入。

## 资产管理工具

| 工具 | 用途 |
|------|------|
| `add_account` | 添加资产账户（name, type） |
| `list_accounts` | 列出所有账户 |
| `snapshot_balance` | 记录某日余额快照（account_id, balance, date） |
| `current_balance` | 各账户最新余额 |
| `balance_trend` | 某账户余额历史趋势（account_id） |
| `total_assets` | 总资产汇总（按类型分组） |
