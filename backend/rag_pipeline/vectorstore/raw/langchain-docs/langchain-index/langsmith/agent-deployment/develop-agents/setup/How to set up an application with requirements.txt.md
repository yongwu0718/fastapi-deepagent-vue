# 如何使用 requirements.txt 设置应用

一个应用必须通过配置文件进行配置，才能部署到 LangSmith（或自托管）。本操作指南介绍了使用 `requirements.txt` 指定项目依赖来设置部署应用的基本步骤。
...
* `pyproject.toml`：如果您喜欢使用 poetry 管理依赖，请查看这篇关于为 LangSmith 使用 `pyproject.toml` 的操作指南。
* monorepo：如果您想部署位于 monorepo 内部的 graph，请参考此仓库中的示例。
...
实现您的 graph。graph 可以在单个文件或多个文件中定义。记下要包含在应用中的每个 `CompiledStateGraph` 的变量名。这些变量名将在后续创建 LangGraph 配置文件时使用。
