...
与指定响应模型的方式相同，在以下任意_路径操作_中，可以使用 `status_code` 参数声明用于响应的 HTTP 状态码：
...
注意，`status_code` 是（`get`、`post` 等）**装饰器**方法中的参数。与之前的参数和请求体不同，不是_路径操作函数_的参数。
***
...
想了解每个状态码的更多信息以及适用场景，请参阅 MDN 的 HTTP 状态码文档。
***
...
稍后在高级用户指南中，你将看到如何返回与此处声明的默认状态码不同的状态码。