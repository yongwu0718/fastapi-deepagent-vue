...
你可以在你的_路径操作函数_中声明一个`Response`类型的参数（就像你可以为cookies和头部做的那样）。

然后你可以在这个_临时_响应对象中设置`status_code`。
```python
...
**FastAPI**将使用这个_临时_响应来提取状态码（也包括cookies和头部），并将它们放入包含你返回的值的最终响应中，该响应由任何`response_model`过滤。
